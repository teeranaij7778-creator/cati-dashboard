...
const App = () => {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4 text-white font-sans">
        <div className="bg-zinc-900 p-10 rounded-[2.5rem] border border-zinc-800 w-full max-sm:w-full text-center shadow-2xl">
          ...
          <form onSubmit={(e) => { e.preventDefault(); if(inputUser==='Admin'&&inputPass==='1234') setIsAuthenticated(true); else setLoginError('Login Failed'); }} className="space-y-4 text-left">
            <input type="text" value={inputUser} onChange={e=>setInputUser(e.target.value)} className="w-full p-4 bg-zinc-800 border border-zinc-700 rounded-2xl outline-none focus:ring-2 focus:ring-red-600 text-white" placeholder="Username" />
            <input type="password" value={inputPass} onChange={e=>setInputPass(e.target.value)} className="w-full p-4 bg-zinc-800 border border-zinc-700 rounded-2xl outline-none focus:ring-2 focus:ring-red-600 text-white" placeholder="Password" />
            {loginError && <p className="text-red-500 text-xs font-bold text-center">Login Credentials Incorrect</p>}
  ...
  const FilterSection = ({ title, items, selectedItems, onToggle, onSelectAll, onClear, maxH = "max-h-40" }) => (
    <div className="space-y-2">
        ...
        <div className={`bg-zinc-800/50 border border-zinc-800/50 rounded-2xl p-2 overflow-y-auto custom-scrollbar ${maxH}`}>
            {items.map(item => (
                <div key={item} onClick={() => onToggle(item)} className={`flex items-center gap-2 p-2 rounded-xl cursor-pointer text-[10px] font-bold mb-1 transition-all ${selectedItems.includes(item) ? 'bg-zinc-700 text-white shadow-lg' : 'hover:bg-zinc-800/50 text-zinc-500'}`}>
  ...
  return (
    <div className="min-h-screen bg-zinc-950 p-4 md:p-8 text-zinc-100 font-sans custom-scrollbar">
      ...
      <aside className={`fixed inset-y-0 right-0 z-50 w-80 bg-zinc-900 shadow-2xl transform transition-transform duration-300 ${isFilterSidebarOpen ? 'translate-x-0' : 'translate-x-full'} overflow-y-auto border-l border-zinc-800 p-6`}>
          <div className="flex items-center justify-between mb-8 pb-4 border-b border-zinc-800">
             <div className="flex items-center gap-2"><div className="p-2 bg-red-600 rounded-lg text-white shadow-lg shadow-red-900/20"><Filter size={20} /></div><h3 className="font-black text-white uppercase italic tracking-tight">ตัวกรอง</h3></div>
             <button onClick={() => setIsFilterSidebarOpen(false)}><X size={20} /></button>
          </div>
          <div className="space-y-8">
             <button onClick={() => { setSelectedMonths([]); setSelectedSups([]); setSelectedAgents([]); setSelectedResults([]); setSelectedTypes([]); setActiveCell({ agent: null, resultType: null }); }} className="w-full py-2.5 text-xs font-black text-red-500 bg-red-600/10 hover:bg-red-600/20 rounded-xl border border-red-600/20 uppercase tracking-widest transition-all">ล้างตัวกรองทั้งหมด</button>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-zinc-900 p-6 rounded-[2.5rem] shadow-xl border border-zinc-800">
          <div className="flex items-center gap-6"><div className="p-3 bg-zinc-800 rounded-2xl border border-zinc-700 shadow-inner"><IntageLogo /></div>
            <div>
              ...
              <div className="text-white text-[10px] font-black uppercase tracking-widest mt-1 flex items-center gap-2 italic">{data.length > 0 ? <><div className="w-1.5 h-1.5 rounded-full bg-red-600 animate-pulse"></div> เชื่อมต่อแล้ว: {data.length} รายการ</> : "OFFLINE"} {lastUpdated && <span className="ml-4 opacity-50"><Clock size={10} className="inline mr-1" />{lastUpdated}</span>}</div>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            <button onClick={() => setIsFilterSidebarOpen(true)} className={`flex items-center gap-2 px-5 py-3 rounded-2xl text-xs font-black shadow-sm transition-all border ${selectedResults.length > 0 || selectedSups.length > 0 || selectedMonths.length > 0 || selectedAgents.length > 0 || selectedTypes.length > 0 ? 'bg-red-600 border-red-500 text-white shadow-lg shadow-red-900/40' : 'bg-zinc-800 border-zinc-700 hover:bg-zinc-700'}`}><Filter size={16} /> ตัวกรอง</button>
        ...
        <div className="flex flex-col lg:flex-row gap-6">
            <div className="bg-zinc-900 p-8 rounded-[3rem] shadow-2xl border border-zinc-800 flex-1 flex flex-col min-h-[300px]">
                <h3 className="font-black text-white flex items-center gap-2 italic text-sm uppercase mb-6"><PieChart size={16} className="text-red-600" /> Case Composition Summary</h3>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-6">
                    {chartData.map(c => (
                        <div key={c.full} className="flex items-center gap-2">
                            ...
                            <span className="text-[9px] text-zinc-400 font-bold truncate uppercase">{c.name}</span>
                        </div>
                    ))}
                </div>
            </div>
        ...
        <div className="bg-zinc-900 rounded-[3rem] shadow-2xl border border-zinc-800 overflow-hidden">
            <div className="p-8 border-b border-zinc-800 bg-zinc-900/50 flex items-center justify-between">
                <div><h3 className="font-black text-white flex items-center gap-2 italic text-lg uppercase tracking-tight"><Users size={20} className="text-red-600" /> สรุปพนักงาน (ID : ชื่อ) x ผลการตรวจ</h3></div>
            </div>
            <div className="overflow-x-auto max-h-[500px] custom-scrollbar">
                <table className="w-full text-left text-sm border-separate border-spacing-0 min-w-[1200px]">
                    <thead className="sticky top-0 bg-zinc-900 z-20 font-black text-white text-[10px] uppercase tracking-widest border-b border-zinc-800 shadow-md">
                        <tr>
                            <th rowSpan="2" className="px-8 py-6 border-b border-zinc-800 border-r border-zinc-800 bg-zinc-900 w-64 text-zinc-100 italic">Interviewer (ID : Name)</th>
                            <th colSpan={RESULT_ORDER.length} className="px-4 py-4 text-center border-b border-zinc-800 bg-zinc-900/40 text-red-600 text-[11px] font-black italic uppercase">QC Result Distribution</th>
                            <th rowSpan="2" className="px-8 py-6 text-center bg-zinc-800 text-white border-b border-zinc-800 border-l border-zinc-800">Total</th>
                        </tr>
                    ...
                    <tbody className="divide-y divide-zinc-800/40 font-bold">
                        ...
                        <tr key={i} className="hover:bg-zinc-800/50 transition-colors group">
                            ...
                            return (
                                <td key={type} className={`px-4 py-5 text-center border-r border-zinc-800 transition-all ${val > 0 ? 'cursor-pointer hover:bg-zinc-700/50 shadow-inner' : ''} ${isActive ? 'bg-red-600/20 ring-2 ring-inset ring-red-600' : ''}`} onClick={() => val > 0 && handleMatrixClick(agent.name, type)}>
        ...
        <div id="detail-section" className="bg-zinc-900 rounded-[3rem] shadow-2xl border border-zinc-800 overflow-hidden scroll-mt-6">
            <div className="p-8 border-b border-zinc-800 flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="space-y-1">
                    <h3 className="font-black text-white uppercase tracking-widest text-xs flex items-center gap-2 italic"><MessageSquare size={16} className="text-red-600" /> รายละเอียดรายเคส</h3>
                    {activeCell.agent && <div className="flex items-center gap-2 mt-2"><span className="px-3 py-1 bg-red-600 text-white text-[9px] font-black rounded-lg uppercase italic animate-pulse">กำลังแสดง: {activeCell.agent}</span><button onClick={() => setActiveCell({ agent: null, resultType: null })}><X size={12}/></button></div>}
                </div>
            ...
            <div className="overflow-auto max-h-[1000px] custom-scrollbar">
                ...
                <thead className="sticky top-0 bg-zinc-900 shadow-md z-10 border-b border-zinc-800 font-black text-white uppercase tracking-widest">
                    <tr><th className="px-8 py-5 border-r border-zinc-800/30">วันที่ / เลขชุด</th><th className="px-8 py-5 border-r border-zinc-800/30">พนักงาน (ID : ชื่อ)</th><th className="px-4 py-5 text-center border-r border-zinc-800/30">ผลสรุป</th><th className="px-8 py-5">QC Comment & Audio</th></tr>
                </thead>
                <tbody className="divide-y divide-zinc-800/30">
                    {detailLogs.length > 0 ? detailLogs.slice(0, 150).map((item) => {
                        ...
                        <React.Fragment key={item.id}>
                            <tr onClick={() => !isEditing && setExpandedCaseId(isExpanded ? null : item.id)} className={`transition-all group cursor-pointer ${isExpanded ? 'bg-zinc-800/50 shadow-inner' : 'hover:bg-zinc-800/40'}`}>
                                ...
                                <td className="px-8 py-6 border-r border-zinc-800/20"><div className="font-black text-white text-sm group-hover:text-red-500 transition-colors flex items-center gap-2">{item.agent} {isExpanded ? <ChevronDown size={14} className="text-zinc-500" /> : <ChevronRight size={14} className="text-zinc-700" />}</div><div className="text-[9px] text-zinc-500 font-bold mt-0.5 italic uppercase font-sans">TYPE: {item.type} &bull; SUP: {item.supervisor} &bull; {item.touchpoint}</div></td>
                                ...
                                <td className="px-8 py-6">
                                    ...
                                    {item.audio && item.audio.includes('http') && (<a href={item.audio} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="mt-2 inline-flex items-center gap-1 text-red-500 hover:text-red-400 font-black text-[9px] uppercase transition-all hover:translate-x-1"><PlayCircle size={14} /> Listen Recording</a>)}
                                </td>
                            </tr>
                            {isExpanded && (
                            <tr className="bg-zinc-950/40 animate-in slide-in-from-top-2 duration-300">
                                ...
                                <div className="flex items-center justify-between mb-8">
                                    <div className="flex items-center gap-4"><div className="p-3 bg-red-600/10 rounded-2xl text-red-600 shadow-inner"><Award /></div><div><h4 className="font-black uppercase italic tracking-widest text-sm text-white">Assessment Detail (ID: {item.questionnaireNo})</h4><p className="text-[9px] text-zinc-500 font-bold uppercase tracking-widest italic">แก้ไขคะแนน P:AB และ สรุปผล M</p></div></div>
                                    <div className="flex gap-2">
                                ...
                                {item.audio && item.audio.includes('http') && (
                                    <div className="mb-8 p-6 bg-zinc-950/50 border border-zinc-800 rounded-[2rem] flex items-center justify-between shadow-inner">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div><p className="text-[10px] font-black uppercase text-zinc-500 tracking-widest">Audio Recording File</p><p className="text-xs font-black text-white">ไฟล์เสียงบันทึกการสัมภาษณ์</p></div>
                                        </div>
                                        <a href={item.audio} target="_blank" rel="noopener noreferrer" className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-2xl text-[10px] font-black uppercase flex items-center gap-2 transition-all shadow-lg shadow-red-900/20"><ExternalLink size={14} /> OPEN PLAYER</a>
                                    </div>
                                )}
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
                                    ...
                                    <div key={eIdx} className={`bg-zinc-900 border p-3 rounded-2xl transition-all ${isEditing ? 'border-red-600/50 ring-1 ring-red-600/20' : 'border-zinc-800'}`}>
                                        ...
                                        {isEditing ? (<div className="relative"><select className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-2 py-1.5 text-[10px] font-black text-white appearance-none outline-none focus:ring-1 focus:ring-red-600" value={evalItem.value} onChange={(e) => { const newEvals = [...editingCase.evaluations]; newEvals[eIdx].value = e.target.value; setEditingCase({...editingCase, evaluations: newEvals}); }}>{SCORE_OPTIONS.map(opt => (<option key={opt} value={opt}>{SCORE_LABELS[opt]}</option>))}</select><ChevronDown size={10} className="absolute right-1 top-1/2 -translate-y-1/2 text-zinc-600 pointer-events-none" /></div>) : (<div className={`text-sm font-black italic tracking-widest ${evalItem.value === '5' || evalItem.value === '4' ? 'text-emerald-500' : (evalItem.value === '1' || evalItem.value === '2') ? 'text-red-500' : 'text-zinc-200'}`}>{SCORE_LABELS[evalItem.value] || evalItem.value}</div>)}
                                    </div>
                                    ))}
                                </div>
                                <div className="mt-8">
                                    ...
                                    {isEditing ? (<textarea className="w-full bg-zinc-950 border border-zinc-800 rounded-[1.5rem] p-6 text-sm italic text-white outline-none min-h-[120px] focus:ring-2 focus:ring-red-600 shadow-inner font-sans" value={editingCase.comment} onChange={e => setEditingCase({...editingCase, comment: e.target.value})}/>) : (<div className="p-4 bg-zinc-800/30 rounded-2xl border border-zinc-800 border-l-4 border-l-red-600 shadow-sm"><p className="text-sm text-zinc-100 font-medium italic leading-relaxed font-sans">"{item.comment || 'ไม่มีคอมเมนต์'}"</p></div>)}
                                </div>
                                </td>
                            </tr>
                            )}
                    ...
                    }) : (<tr><td colSpan={4} className="px-8 py-24 text-center text-white font-black uppercase italic tracking-widest text-lg opacity-20 italic">ไม่พบข้อมูลตามเงื่อนไขที่เลือก</td></tr>)}
                </tbody>
                </table>
            </div>
        </div>
        ...
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4">
            <div className="bg-zinc-900 w-full max-w-2xl rounded-[3rem] p-10 border border-zinc-800 shadow-2xl relative animate-in zoom-in duration-300">
                ...
                <div className="space-y-6">
                    <div className="space-y-2"><label className="text-[10px] font-black text-zinc-100 uppercase tracking-widest pl-2 italic">Google Sheets CSV Link</label><input type="text" className="w-full px-6 py-4 bg-zinc-950 border border-zinc-800 rounded-2xl text-xs font-bold text-white outline-none focus:ring-2 focus:ring-red-600 shadow-inner" value={sheetUrl} onChange={e=>setSheetUrl(e.target.value)} /></div>
                    <div className="space-y-2"><label className="text-[10px] font-black text-zinc-100 uppercase tracking-widest pl-2 italic">Apps Script API Link</label><input type="text" className="w-full px-6 py-4 bg-zinc-950 border border-zinc-800 rounded-2xl text-xs font-bold text-white outline-none focus:ring-2 focus:ring-red-600 shadow-inner" value={appsScriptUrl} onChange={e=>{setAppsScriptUrl(e.target.value); localStorage.setItem('apps_script_url', e.target.value);}} /></div>
                    ...
                    {error && <p className="text-center text-[10px] text-red-500 font-black mt-2 uppercase italic animate-pulse">{error}</p>}
                </div>
            </div>
        </div>
      )}
